 ...
 *
 * Gathers:
 * - Project identity (CLAUDE.md, package.json, etc.)
 * - Git state (branch, status, recent commits, stash, diff stats)
 * - Recently modified files (via git or filesystem)
 * - Claude Code session hints (last-session, .remember/)
 *
 * Usage: node context-collector.mjs [cwd]
 */
...
function getGitInfo(cwd) {
  ...
  info.changeSummary = { staged, unstaged, untracked: info.statusFiles.filter(l => l.startsWith('??')).length };
...
function getProjectInfo(cwd) {
  ...
  if (existsSync(join(cwd, 'tsconfig.json'))) info.type = 'TypeScript';
...
// -- Recently modified files (via git or fs) --
...
function formatContextAsMarkdown(ctx) {
  ...
  if (ctx.git.isRepo) {
    ...
    if (ctx.git.stashCount) lines.push(`- **暂存 (stash)**: ${ctx.git.stashCount} 项`);
